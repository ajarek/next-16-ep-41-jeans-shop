...
export default function ProductList({ onProductClick, selectedGender, setSelectedGender }: ProductListProps) {
  ...
  return (
    <section id="shop-catalog" className="bg-[#FDFCFB] py-16 border-b border-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12 space-y-3">
          ...
          <p className="text-xs font-bold uppercase tracking-wider text-stone-500 max-w-md mx-auto">
            Nasze jeansy to klasyka połączona z bezkompromisowym designem. Znajdź swój fason i ciesz się najwyższą jakością.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-[260px] shrink-0 space-y-6">
            ...
            <div className="flex lg:hidden justify-between items-center bg-white p-4 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
              ...
              {(selectedCategory !== 'all' || selectedSize !== 'all' || selectedStyle !== 'all' || searchQuery !== '' || selectedGender !== 'all') && (
                <button onClick={() => { resetFilters(); setSelectedGender('all'); }} className="text-[11px] text-[#E11D48] font-black uppercase tracking-wider hover:underline">
                  Resetuj
                </button>
              )}
            </div>
            ...
            <div className={`space-y-6 ${showFiltersMobile ? 'block' : 'hidden lg:block'}`}>
              ...
              <div className="border-2 border-black bg-[#F8F7F3] p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] space-y-6">
                ...
                <div className="hidden lg:flex justify-between items-center pb-2 border-b border-black">
                  ...
                  {(selectedCategory !== 'all' || selectedSize !== 'all' || selectedStyle !== 'all' || searchQuery !== '' || selectedGender !== 'all') && (
                    <button onClick={() => { resetFilters(); setSelectedGender('all'); }} className="text-[10px] text-stone-500 hover:text-red-600 font-bold uppercase tracking-wider transition-colors">
                      Wyczyść
                    </button>
                  )}
                </div>
                ...
                <div className="space-y-2.5">
                  <h4 className="text-xs font-black uppercase tracking-wider text-[#1A1A1A]">Płeć</h4>
                  <div className="flex flex-col gap-1.5">
                    {['all', 'Woman', 'Man'].map((gender) => (
                      ...
                      >
                        {gender === 'all' ? 'Wszystko' : gender === 'Woman' ? 'Kolekcja Damska' : 'Kolekcja Męska'}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-1.5">
                    {categories.map((cat) => (
                      ...
                      >
                        <span className="font-bold uppercase text-[10px] tracking-wider">{cat.name}</span>
                ...
                <div className="space-y-2.5">
                  <h4 className="text-xs font-black uppercase tracking-wider text-[#1A1A1A]">Rozmiar (W)</h4>
                ...
                <div className="space-y-2.5">
                  <h4 className="text-xs font-black uppercase tracking-wider text-[#1A1A1A]">Fason / Krój</h4>
                  <div className="space-y-1.5">
                    <button
                    ...
                    >
                      <span className="font-bold uppercase text-[10px] tracking-wider">WSZYSTKIE STYLE</span>
                    </button>
                    {STYLES.map((style) => (
                      <button
                        key={style}
                      ...
                      >
                        <span className="font-bold uppercase text-[10px] tracking-wider">
                          {style === 'Casual' ? 'Casual (Codzienny)' :
                           style === 'Elegant' ? 'Elegant (Elegancki)' :
                           style === 'Distressed' ? 'Distressed (Przecierany)' :
                           style === 'High Waist' ? 'High Waist (Wysoki Stan)' :
                           style === 'Classic' ? 'Classic (Klasyczny)' : style}
                        </span>
                      </button>
                    ))}
                  </div>
          ...
          <div className="flex-1 space-y-6">
            ...
            <div className="flex justify-between items-center text-xs font-bold uppercase tracking-wider text-stone-500">
              <p>Znaleziono: <strong className="text-black font-black">{filteredProducts.length}</strong> produktów</p>
              {searchQuery && (
                <p>Wyniki dla: <span className="italic font-black text-[#E11D48]">&quot;{searchQuery}&quot;</span></p>
              )}
            </div>
            ...
            {loadingProducts ? (
              ...
              <div className="text-center py-20 bg-white border-2 border-dashed border-black">
                ...
                <p className="text-xs text-stone-500 mt-1 max-w-xs mx-auto font-medium">
                  Spróbuj zmienić filtry rozmiaru, stylu lub wyczyść zapytanie wyszukiwania, aby zobaczyć całą kolekcję.
                </p>
                <button
                  onClick={() => { resetFilters(); setSelectedGender('all'); }}
