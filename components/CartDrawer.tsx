            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#FDFCFB]">
              {cart.length === 0 ? (
                <div className="text-center py-24 space-y-4">
                  ...
                  <div>
                    <h4 className="text-base font-black uppercase tracking-tight text-stone-950">Koszyk jest pusty</h4>
                    <p className="text-xs font-bold uppercase tracking-wide text-stone-500 mt-1.5 max-w-[200px] mx-auto">
                      Nie dodałeś jeszcze żadnych jeansowych ubrań do swojego koszyka.
                    </p>
                  </div>
                  <button
                    onClick={onClose}
                      ...
                      <div className="flex-1 flex flex-col justify-between py-0.5">
                        <div className="space-y-0.5">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-widest text-[#2B3E50]">
                            Rozmiar: <strong className="text-[#E11D48]">{item.size}</strong> • {item.product.style}
                          </p>
                        </div>
              ...
              <div className="border-t-2 border-black bg-[#F8F7F3] p-6 space-y-4">
                ...
                <form onSubmit={handleApplyCoupon} className="space-y-1.5">
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <Tag size={13} className="absolute left-3 top-3 text-[#1A1A1A]" />
                      <input
                        type="text"
                  ...
                  {couponError && <p className="text-[10px] text-red-600 font-bold pl-1 uppercase tracking-wide">{couponError}</p>}
                  {couponSuccess && <p className="text-[10px] text-emerald-600 font-bold pl-1 uppercase tracking-wide">{couponSuccess}</p>}
                  ...
                  {promoCode && (
                    <div className="flex items-center justify-between bg-rose-50 text-[#E11D48] text-[9px] font-black uppercase py-1 px-2.5 border-2 border-black mt-1">
                      ...
                      <button type="button" onClick={removePromoCode} className="text-black font-black uppercase underline hover:text-[#E11D48]">
                        Usuń
                      </button>
                    </div>
                  )}
                ...
                <div className="space-y-1.5 text-xs font-bold uppercase tracking-wider text-stone-700">
                  ...
                  <div className="flex justify-between items-baseline text-stone-900">
                    <span className="text-sm font-black">Do zapłaty (Razem)</span>
                    <span className="text-lg font-mono font-black">${total.toFixed(2)}</span>
                  </div>
                </div>
